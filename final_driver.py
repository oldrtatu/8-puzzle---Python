...
class Play:
    ...
    def solve_bfs(self):
        ...
        while(frontier):
            check = frontier.pop()
            explored.append(check)
            check.p_print()
            if(check.root in answer):
                check.p_print()
                break
            check.children.reverse()
            for i in check.children:
                if(self.exist(i.root, frontier) and self.exist(i.root, explored)):
                    frontier.append(i)

        while(check.parent is not None):
            self.path.append(check.position)
            check = check.parent
        print(self.path)
...
a = Board([1,2,5,3,4,0,6,7,8])
inn = open("puzzle_test1.pickle","rb")
d = pickle.load(inn)
inn.close()
c = Play(d,'dfs')
